...
- Planner 算法调度，包含经典搜索、基础 PSO、RS-APSO、RRT、RRT-Connect 和 RRT*；JSON v1 示例、序列化、字符串解析、嵌入式示例地图、SVG 导出、CLI demo，以及支持 JSON 文件、JSON 字符串和示例名输入的 benchmark runner。
...
- 第四阶段：LPA*、D* Lite，并继续扩展基础 RRT/RRT-Connect/RRT* 的 benchmark 和可视化案例。
