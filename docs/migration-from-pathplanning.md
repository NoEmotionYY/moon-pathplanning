...
- 统一 Planner、JSON 示例 schema 与 SVG/HTML 可视化导出。

## 后续迁移能力

- 增量规划：LPA* 和 D* Lite 阶段入口已接入 Planner，当前保留 `g/rhs/key` 状态和变化单元记录，后续继续增强为完整增量复用。
- 采样规划：基础 RRT、RRT-Connect 和 RRT* 已开始，已接入静态 benchmark CSV 和 SVG/HTML 对比导出。
