...
`RsApso`、`Rrt` 和 `RrtConnect`。`Pso` 与 `RsApso` 会在内部构建默认区域搜索候选集并返回统一 `PathResult`；
如果需要迭代次数、候选数量和适应度等附加信息，应直接调用 `@swarm.pso_plan()` 或
`@swarm.rs_apso_plan()`；如果需要 RRT/RRT-Connect 迭代次数和采样树节点数，应直接调用
`@continuous.rrt_plan()` 或 `@continuous.rrt_connect_plan()`。
...
`rrt_plan(map, options)` 和 `rrt_connect_plan(map, options)` 组成；`rrt_plan()` 会在可行走栅格中心采样，选择最近树节点，
...
成功后把两棵树路径拼接并做快捷平滑。Planner 的 `Rrt` 与 `RrtConnect` 算法类型使用默认 RRT 参数；
后续 RRT* 或更丰富路径平滑后处理可继续复用
同一线段可见性入口。
