...
LPA* 与 D* Lite 当前先提供 MoonBit 原生阶段入口，用于统一调度、测试增量变化场景并保留 `g/rhs/key` 状态结构。阶段版会在地图变化后重新生成一致的搜索状态，不复用上一轮 open list 或优先队列，因此不能等同于具备增量状态复用的 LPA*/D* Lite 规划器。后续可以继续演进为复用上一轮 open list 的增量实现：
