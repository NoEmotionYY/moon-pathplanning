...
和进入碰撞半径后的跳跃修正测试。基础 RRT 已开始提供连续空间采样规划入口，后续可继续补 RRT-Connect、RRT* 和 benchmark 对比。

### 第四阶段：连续空间几何与基础 RRT

已开始新增 `src/continuous` 包，为采样规划和路径平滑后处理提供静态几何能力，并补充基础 RRT：
...
Planner 已新增 `Rrt` 算法类型，使用默认 RRT 参数返回统一 `PathResult`。后续 RRT-Connect
和 RRT* 可复用该可见性入口判断采样点之间是否能直连。
...
基础 RRT 当前已接入 Planner，但尚未加入 benchmark CSV；后续可继续补 RRT、RRT-Connect、RRT* 的路径长度、平滑度、节点数和耗时对比。
