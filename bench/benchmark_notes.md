...
72 项测试全部通过。后续 benchmark runner 应在输出中记录同类工具链版本和目标后端信息。
...
`src/continuous` 已提供静态线段可见性、路径快捷平滑、基础 RRT、RRT-Connect 和 RRT* 采样规划，后续 benchmark
可增加三类采样算法的路径长度、平滑度、采样树节点数和耗时对比行。
