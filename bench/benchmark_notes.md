...
47 项测试全部通过。后续 benchmark runner 应在输出中记录同类工具链版本和目标后端信息。
...
重复运行次数、总耗时和平均耗时。runner 也输出 `dynamic_5x1` 动态避障场景，覆盖静态
A 星基线、整数速度动态修正、边界往复修正和连续坐标时间步修正。CLI 已能在 native 后端
读取 JSON v1 地图文件；下一步应补 benchmark 文件输入参数和更大规模的动态场景。
